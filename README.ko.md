...
- **에이전트 작업형 대화 화면** — 하단 입력창, 작은 점으로 시작하는 에이전트 출력, 위아래 여백과 고대비 배경으로 분리한 사용자 입력, 입력창 위 실시간 작업 표시와 아래 제공자 상태선을 표시합니다.
...
| `←` / `→`, `Home` / `End` | 입력창 안에서 커서 이동 (`←`는 대화 입력이 비었을 때만 목록으로 복귀) |
| `Backspace` / `Delete` | 입력 커서 앞 또는 현재 글자 삭제 |
...
| 마우스 휠 | 목록 선택 이동 또는 대화를 세 줄씩 스크롤 |
| `Esc` | 빠른 답장 취소, 입력 내용 지우기 또는 빈 대화 입력창에서 진행 중인 턴 중단 |
